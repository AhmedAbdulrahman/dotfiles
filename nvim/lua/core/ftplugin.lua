...
M.extend = function(name, new_config)
  ...
  conf.ignore_win_opts =
    coalesce(new_config.ignore_win_opts, conf.ignore_win_opts)
  configs[name] = conf
  if did_setup then
    M.reapply_all_bufs()
  end
...
M.apply_win = function(name, winid)
  ...
  local has_prev, prev_win_opts =
    pcall(vim.api.nvim_win_get_var, winid, 'ftplugin_set_opts')
  if has_prev then
    for _, prev_opt in ipairs(prev_win_opts) do
      if not win_overrides[prev_opt] then
        ...
        if not ok then
          vim.notify(
            string.format(
              'Error restoring window option %s = %s: %s',
              prev_opt,
              get_default_opt(winid, prev_opt),
              err
            ),
