...
local function blame_syntax()
  ...
  for lnum = 1, vim.fn.line('$') do
    local orig_hash = vim.fn.matchstr(
      vim.fn.getline(lnum),
      [[^\^\=[*?]*\zs\x\{6\}]]
    )
    local hash = orig_hash
    hash = vim.fn.substitute(
      hash,
      [[\(\x\)\x]],
      [[\=submatch(1).printf("%x", 15-str2nr(submatch(1),16))]],
      'g'
    )
    hash = vim.fn.substitute(
      hash,
      [[\(\x\x\)]],
      [[\=printf("%02x", str2nr(submatch(1),16)*3/4+32)]],
      'g'
    )
    if hash ~= '' and orig_hash ~= '000000' and seen[hash] == nil then
      seen[hash] = 1
      if vim.wo.t_Co == '256' then
        local colors = vim.fn.map(
          vim.fn.matchlist(orig_hash, [[\(\x\)\x\(\x\)\x\(\x\)\x]]),
          'str2nr(v:val,16)'
        )
        local r = colors[2]
        local g = colors[3]
    ...
    for hash_value, cterm in pairs(hash_colors) do
      if
        ...
        or vim.fn.hash('termguicolors') and vim.wo.termguicolors
      then
        vim.cmd(
          'hi GitNvimBlameHash'
            .. hash_value
...
local function on_blame_commit_done(commit_hash, lines)
  ...
  vim.fn.search(
    [[^diff .* b/\M]] .. vim.fn.escape(blame_state.relative_path, '\\') .. '$',
    'W'
  )
end
...
function M.blame_commit()
  ...
  local commit_hash = git.run_git_cmd(
    'git -C '
      ...
      .. ' --literal-pathspecs rev-parse --verify '
      .. commit
      .. ' --'
  )
