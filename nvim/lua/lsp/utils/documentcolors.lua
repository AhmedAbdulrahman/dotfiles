...
local function lsp_color_to_hex(color)
  ...
  return bit.tohex(
    bit.bor(
      bit.lshift(to256(color.red), 16),
      bit.lshift(to256(color.green), 8),
      to256(color.blue)
    ),
    6
  )
...
local function make_highlight_name(rgb, mode)
  return table.concat(
    { HIGHLIGHT_NAME_PREFIX, HIGHLIGHT_MODE_NAMES[mode], rgb },
    '_'
  )
end
...
local function create_highlight(rgb_hex, options)
  ...
  if mode == 'foreground' then
    vim.api.nvim_command(
      string.format('highlight %s guifg=#%s', highlight_name, rgb_hex)
    )
  else
...
local function buf_set_highlights(bufnr, colors, options)
  ...
  for _, color_info in pairs(colors) do
    if color_info ~= nil or type(color_info) ~= 'number' then
      ...
      local end_col = options.single_column and start_col + 1
        or range['end'].character
...
function M.buf_attach(bufnr, options)
  ...
  local trigger_update_highlight, timer =
    require('lsp/utils/defer').debounce_trailing(
      M.update_highlight,
