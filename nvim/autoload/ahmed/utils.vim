...
function! ahmed#utils#ZoomToggle() abort
...
function! ahmed#utils#SynStack() abort
  if !exists('*synstack')
    return
  endif
...
function! ahmed#utils#Preserve(command) abort
...
function! ahmed#utils#ClearRegisters() abort
...
function! ahmed#utils#HtmlEscape() abort
  silent s/&/\&amp;/eg
  silent s/</\&lt;/eg
  silent s/>/\&gt;/eg
endfunction

function! ahmed#utils#HtmlUnEscape() abort
  silent s/&lt;/</eg
  silent s/&gt;/>/eg
  silent s/&amp;/\&/eg
endfunction

function! ahmed#utils#OpenFileFolder() abort
  silent call system(ahmed#utils#open() . ' '.expand('%:p:h:~'))
endfunction

function! ahmed#utils#should_strip_whitespace(filetypelist) abort
  return index(a:filetypelist, &filetype) == -1
endfunction

function! ahmed#utils#open() abort
  " Linux/BSD"
...
function! ahmed#utils#openMarkdownPreview() abort
...
function! ahmed#utils#is_git() abort
  silent call system('git rev-parse')
  return v:shell_error == 0
endfunction

function! ahmed#utils#helptopic() abort
  let col = col('.') - 1
